...
def profile():
    ...
    Requires current_password for any security-related changes (email or new password).
    Viewers cannot persist profile changes to DB.
    Fetches and displays user-specific order statistics.
    """
    msg = ''
    order_count = 0
    total_quantity = 0
    ...
    # Calculate user statistics if logged in
    if 'loggedin' in session and 'email' in session:
        ...
        if stats_conn:
            try:
                ...
                # 1. Count total orders for the user
                ...
                # 2. Sum quantity (deposit) and disquantity (recycled)
                # NOTE: Changed column names to reflect 'deposit' and 'recycled' as per HTML context (₿ เงินมัดจำ, คะแนน)
                # Assuming 'quantity' is deposit and 'disquantity' is recycled/rated.
                cursor.execute('SELECT SUM(quantity), SUM(disquantity) FROM tbl_order WHERE email = %s', (user_email,))
                sum_result = cursor.fetchone()
                
                db_total_quantity = sum_result[0] if sum_result and sum_result[0] is not None else 0
                db_total_disquantity = sum_result[1] if sum_result and sum_result[1] is not None else 0
                
                total_quantity = int(db_total_quantity)

                # 3. Calculate recycling rate: (100 / total_quantity) * disquantity
                if db_total_quantity > 0:
                    # Ensure total_quantity and total_disquantity are floats for division
    ...
    if request.method == 'POST':
        if 'loggedin' not in session:
            ...
            return redirect(url_for('login')) # สมมติว่ามีหน้า login

        # Basic form data validation
        if not all(k in request.form for k in ['firstname', 'lastname', 'email']):
            flash('ข้อมูลที่จำเป็นไม่ครบถ้วน', 'danger')
        ...
        new_password = request.form.get('password') # New password (optional)
        current_password = request.form.get('current_password') # Current password (required for security change)

        # Determine if a security field (email or password) is being updated
        ...
        if update_conn:
            try:
                cursor = update_conn.cursor(dictionary=True) # Use dictionary=True for fetching user data

                # --- 1. Security Check (Current Password) ---
                if needs_security_check or (current_password and len(current_password.strip()) > 0):
                    # Check if current_password is provided for a security change
                    if not current_password or len(current_password.strip()) == 0:
                        flash('ต้องกรอกรหัสผ่านปัจจุบันเพื่อบันทึกการเปลี่ยนแปลงข้อมูลความปลอดภัย/อีเมล!', 'danger')
                        return redirect(url_for('profile'))

                    # Verify current password
                    # IMPORTANT: In a real app, you MUST use a secure hashing library (like bcrypt)
                    # For this example, we assume the stored password is plain text or an unhashed value as in tbl_users.sql
                    cursor.execute('SELECT password FROM tbl_users WHERE id = %s', (session['id'],))
                    user_data = cursor.fetchone()
                    
                    # Replace with proper hashing check: if not check_password_hash(user_data['password'], current_password):
                    if user_data is None or user_data['password'] != current_password: 
                        flash('รหัสผ่านปัจจุบันไม่ถูกต้อง กรุณาลองใหม่อีกครั้ง!', 'danger')
                        return redirect(url_for('profile'))


                # --- 2. New Email Duplication Check ---
                if is_email_changed:
                ...
                if is_password_set:
                    # In a real app, hash new_password here: hashed_password = generate_password_hash(new_password)
    ...
    # Render for GET request or after a failed POST that didn't redirect
